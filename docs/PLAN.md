...
9. ▶️ **Prochain** : saisir le vrai contenu dans le studio (coaches, schedule, testimonials, fighters) ; puis **E6 (DTV app)** — trancher **Q1** storage avant. Backlog blog E8 : "best muay thai camps in thailand" (70/mo listicle), "Chiang Mai vs Phuket". Acheter les domaines (Q5). Env prod à poser : `RESEND_API_KEY`, `CONTACT_EMAIL` (+ `CONTACT_FROM` après vérif domaine Resend), `APIFY_TOKEN`.

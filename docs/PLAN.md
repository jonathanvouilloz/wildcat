...
9. Données réelles manquantes : email + horaires (`src/config/site.ts` + `nav_util_open_today`) — `TODO(real data)`. Redirect `/muay-thai-training` → `/stay-train` (E7/E10). ~~Pillar FR : title/meta à raccourcir~~ (réglé par le re-cadrage E5c — title 50ch / meta 143ch).

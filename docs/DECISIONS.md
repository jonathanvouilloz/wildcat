...
### 2026-08-28 | Maillage service → blog : composant `ReadNext` résolu par `translationKey`
- **Contexte** : audit du build — **2 liens service → blog dans tout le site**, dont un seul écrit à la main (`dtv-visa/long-stay-training.astro`), et **7 articles EN orphelins** (0 lien éditorial entrant), dont le dernier publié `dtv-visa-new-requirements-2026`. Le maillage était unidirectionnel : le blog nourrissait les pages service, l'inverse jamais. Cause réelle : poser un lien service → blog coûtait 2 clés de message par lien **et** un `lang === 'en'` codé en dur, parce que les slugs blog sont traduits par locale et que le FR n'a que 2 articles.
- **Choix** : **`ReadNext.astro`** alimenté par des **`translationKey`**, résolus par **`getPostsByKeys(lang, keys)`** qui **omet les clés absentes de la locale** ; le bloc ne rend rien sous `min` articles (défaut 2). Conséquences : **3 clés de message au total** et non 3 par lien (l'ancre est le titre de l'article, localisé par nature) ; **zéro `lang === 'en'`** et zéro 404 possible ; **auto-réparant** — une traduction FR publiée fait apparaître le lien sans toucher au code. Posé sur 13 pages. En parallèle, **`src/lib/blog-cards.ts`** extrait `toBlogCards()`, qui était **copié à l'identique dans 3 fichiers**.
- **Alternatives** : liens `<a>` inline en prose partout — écarté comme forme unique (meilleur signal SEO mais ~12 clés de message EN+FR et placement manuel) ; slugs codés en dur dans le composant — écarté (un slug traduit par locale rend l'appelant responsable de la locale, c'est exactement ce qui produisait les 404 FR corrigés le même jour) ; garder le `lang === 'en'` — écarté (fige le FR pour toujours).
- **Le signal fort reste in-body** : 16 liens éditoriaux ajoutés dans le corps des articles hôtes, placés là où le sujet était **déjà mentionné** (comptage préalable des occurrences). 0 orphelin restant, 58 liens article → article, 37 liens service → blog. État complet et tableau de placement → `docs/maillage-blog.md`.

### 2026-07-24 | Routeur intake : `process.env` pour les secrets serverless + Telegram exhaustif
- **Contexte** : 1re route serverless du site (`src/pages/api/inquiry.ts`, `prerender=false`) sur un projet 100 % SSG. Le draft du feature file lisait les secrets (`TELEGRAM_*`, `RESEND_API_KEY`, `MISTRAL_API_KEY`) via `import.meta.env` — le pattern par défaut du repo, mais qui **inline la valeur au build** (undefined à la requête) et **ne peuple pas non plus le dev local** (Vite ne met pas `.env` dans `process.env`).
- **Choix** : (1) secrets lus via **`process.env.X ?? import.meta.env.X`** — `process.env` = runtime Node Vercel (source de vérité prod), fallback `import.meta.env` = dev local via `.env`. (2) **Telegram devient la source complète** (nom + email + **téléphone** + tous champs d'intent + message intégral), coordonnées incluses — **renverse** la règle « pas de coordonnées dans TG » du plan, car Meaw ne consulte jamais le mail Web3Forms. (3) **Auto-réponse Resend uniquement sur intent structuré** (`class`/`scooter`), jamais sur la sortie LLM ni `general` ; Mistral ne fait que *trier* le résidu `general`, sur message **redacté** (PII masquée avant le modèle).
- **Alternatives** : `astro:env` typé — écarté (net-new, aucun schéma env dans le repo, surdimensionné pour 4 vars) ; `import.meta.env` seul — écarté (casse le dev local + fragile en prod) ; garder Telegram minimal + coordonnées dans le mail — écarté (Meaw ne lit pas les mails).
- **Portée** : repo (`src/pages/api/inquiry.ts`, `src/scripts/wa-form.ts`, `.env.example`). Gotchas : redémarrer `npm run dev` après ajout de clés (Vite lit `.env` au boot) ; Astro `checkOrigin` (défaut Astro 6) rejette les POST form cross-origin en 403 (le fetch same-origin passe) ; échec Resend silencieux tant que le domaine n'est pas vérifié.
...
- **Piège du conteneur géant (2026-08-28)** : le ratio d'intersection max atteignable vaut `hauteur de root / hauteur de l'élément`. Un conteneur plus haut que ~6,7 viewports ne franchit donc **jamais** `threshold: 0.15` — aucun callback, `.in-view` jamais posée, contenu figé à `opacity: 0` pour toujours. Payé sur `/blog` : `data-animate-stagger` sur un `<ul>` de 23 cartes, invisible en 1 et 2 colonnes (mobile + tablette), visible en 3 colonnes. Deux conséquences gravées : **au-delà de ~8 items, on anime les items, pas le conteneur** (le délai cumulé `--stagger-i × 70ms` atteignait 1,5 s sur la dernière carte, contre les 0,3 s max de `DESIGN.md` §9) ; et l'observer porte désormais un `threshold: [0, 0.15]` + une échappatoire `rootBounds.height < 0.15 × hauteur élément` pour que ce cas reste révélable au lieu de disparaître en silence. Corollaire : un conteneur de stagger dont un filtre masque tous les enfants tombe à hauteur 0 et ne peut plus jamais être révélé (`BlogFilters`).

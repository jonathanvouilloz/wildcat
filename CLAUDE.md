...
**Phase : E1→E5 complets + satellites DTV (E5b). Prochain : contenu studio, vérif faits DTV [À VÉRIFIER], puis E6 (DTV app) ou satellites Stay & Train.**
...
- [ ] Questions ouvertes : **Q1** storage DTV (avant E6), **Q5** domaine. Env prod à poser : `RESEND_API_KEY`, `CONTACT_EMAIL`. Redirect `/muay-thai-training` → `/stay-train` (E7/E10). Faits visa `[À VÉRIFIER]` des briefs à confirmer sur source officielle avant prod.
- [ ] Prochain : saisir le vrai contenu dans le studio (coaches/schedule/testimonials/fighters) ; puis **E6 DTV app** ou satellites Stay & Train (batch 2)

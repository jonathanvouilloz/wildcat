...
- [ ] Questions ouvertes : **Q1** storage DTV (avant E6), **Q5** domaine. Env prod à poser : `RESEND_API_KEY`, `CONTACT_EMAIL`. Redirect `/muay-thai-training` → `/stay-train` (E7/E10). Faits `[À VÉRIFIER]` avant prod : visa DTV (source officielle) · ✅ adresse confirmée (Nong Kwai, Hang Dong — copy camp ancrée + **map embed `/contact`** via `site.contact.mapEmbed`) · ✅ gants+bandes prêtés confirmés · ⏳ âge mini kids (~6 ans estimé, à valider avec Meaw — aucun âge publié).
- [ ] Prochain : trancher le **hero home** (explorations A/B/C/D2 sur `/styleguide`, commit `feat(styleguide)`) ; saisir le vrai contenu dans le studio (coaches/schedule/testimonials/**fighters du livre d'or**) ; puis **E6 DTV app** ou satellites Stay & Train (batch 2 + re-ciblage FR "camp/stage")
